...
- **配置常量**：
  ```python
  HEALTH_CHECK_INTERVAL = 180        # 3 分钟定时检测
  ...
  LIGHT_RESTART_THRESHOLD = 1       # 第 1～9 次重启 Bridge
...
累计失败次数 +1
  ↓
失败次数 ≥ 10？
  ├─ 是 → 重置 miclaw + 重新注入 Bridge
  └─ 否 → 向 miclaw 下发 `重启之前运行的bridge` 指令
             ↓
         保留失败计数
             ↓
         等待 15 秒快速复检
...
- 检测频率：3 分钟/次
- 日消耗：100 × 20 × 24 = 48,000 tokens/账号/天
- 10 个账号：约 480,000 tokens/天

### 网络消耗
- 单次请求：约 1 KB
- 单次响应：约 0.5 KB
- 日消耗：1.5 KB × 480 = 720 KB/账号/天
